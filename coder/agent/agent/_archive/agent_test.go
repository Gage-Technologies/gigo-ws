...
func TestAgent_Stats_SSH(t *testing.T) {
	...
	conn, _, stats, _ := setupAgent(t, agentsdk.WorkspaceAgentMetadata{}, 0)
...
func TestAgent_Stats_ReconnectingPTY(t *testing.T) {
	...
	data, err := json.Marshal(agentsdk.ReconnectingPTYRequest{
		Data: "echo test\r\n",
	})
	require.NoError(t, err)
...
func TestAgent_SessionExec(t *testing.T) {
	t.Parallel()
	session := setupSSHSession(t, agentsdk.WorkspaceAgentMetadata{})
...
func TestAgent_SFTP(t *testing.T) {
	...
	//nolint:dogsled
	conn, _, _, _ := setupAgent(t, agentsdk.WorkspaceAgentMetadata{}, 0)
	sshClient, err := conn.SSHClient(ctx)
	require.NoError(t, err)
	defer sshClient.Close()
...
func TestAgent_StartupScript(t *testing.T) {
	...
	content := "output"
	//nolint:dogsled
	_, _, _, fs := setupAgent(t, agentsdk.WorkspaceAgentMetadata{
...
func TestAgent_Lifecycle(t *testing.T) {
	...
	t.Run("Timeout", func(t *testing.T) {
		...
		_, client, _, _ := setupAgent(t, agentsdk.WorkspaceAgentMetadata{
			StartupScript:        "sleep 10",
			StartupScriptTimeout: time.Nanosecond,
		}, 0)

		want := []models.WorkspaceAgentState{
			models.WorkspaceAgentStateStarting,
			models.WorkspaceAgentStateStartTimeout,
		}

		var got []models.WorkspaceAgentState
	...
	t.Run("Error", func(t *testing.T) {
		...
		want := []models.WorkspaceAgentState{
			models.WorkspaceAgentStateStarting,
			models.WorkspaceAgentStateStartError,
		}
	...
	t.Run("Ready", func(t *testing.T) {
		...
		want := []models.WorkspaceAgentState{
			models.WorkspaceAgentStateStarting,
			models.WorkspaceAgentStateReady,
		}
...
func TestAgent_Speedtest(t *testing.T) {
	...
	//nolint:dogsled
	conn, _, _, _ := setupAgent(t, agentsdk.WorkspaceAgentMetadata{
		DERPMap: derpMap,
	}, 0)
	defer conn.Close()
...
func TestAgent_Reconnect(t *testing.T) {
	...
	agentID := uuid.New()
	statsCh := make(chan *agentsdk.AgentStats)
	derpMap := tailnettest.RunDERPAndSTUN(t)
	client := &client{
		t:       t,
		agentID: agentID,
...
func TestAgent_WriteVSCodeConfigs(t *testing.T) {
	...
	client := &client{
		...
		statsChan:   make(chan *agentsdk.AgentStats),
		coordinator: coordinator,
	}
...
func setupSSHCommand(t *testing.T, beforeArgs []string, afterArgs []string) *exec.Cmd {
	//nolint:dogsled
	agentConn, _, _, _ := setupAgent(t, agentsdk.WorkspaceAgentMetadata{}, 0)
...
func setupSSHSession(t *testing.T, options agentsdk.WorkspaceAgentMetadata) *ssh.Session {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	//nolint:dogsled
...
func setupAgent(t *testing.T, metadata agentsdk.WorkspaceAgentMetadata, ptyTimeout time.Duration) (
	*agentsdk.AgentConn,
	*client,
	<-chan *agentsdk.AgentStats,
	afero.Fs,
) {
	...
	agentID := uuid.New()
	statsCh := make(chan *agentsdk.AgentStats, 50)
...
type client struct {
	...
	agentID            uuid.UUID
	metadata           agentsdk.WorkspaceAgentMetadata
	statsChan          chan *agentsdk.AgentStats
	coordinator        tailnet.Coordinator
	lastWorkspaceAgent func()

	mu              sync.Mutex // Protects following.
	lifecycleStates []models.WorkspaceAgentState
}

func (c *client) InitializeAgent(_ context.Context) (agentsdk.WorkspaceAgentMetadata, error) {
...
func (c *client) AgentReportStats(ctx context.Context, _ slog.Logger, stats func() *agentsdk.AgentStats) (io.Closer, error) {
...
func (c *client) getLifecycleStates() []models.WorkspaceAgentState {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.lifecycleStates
}

func (c *client) PostWorkspaceAgentState(_ context.Context, req agentsdk.PostWorkspaceAgentLifecycleRequest) error {
	c.mu.Lock()
	defer c.mu.Unlock()
